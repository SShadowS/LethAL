...
`tables.itest.ts`'s `EXPECTED` now asserts the live-measured **109 killed / 17 survived / 10
...
109 + 17 + 10 = 136 recorded mutants come back from the live gate (126 of them SCORED; the 10 no-coverage are excluded from the score). Every shape below exists because its ABSENCE lets a
