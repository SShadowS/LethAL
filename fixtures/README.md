...
`tables.itest.ts`'s `EXPECTED` now asserts the live-measured **201 killed / 34 survived / 10
...
201 + 34 + 10 = 245 recorded mutants come back from the live gate (235 of them SCORED; the 10 no-coverage are excluded from the score). Every shape below exists because its ABSENCE lets a
