...
`tables.itest.ts`'s `EXPECTED` now asserts the live-measured **194 killed / 32 survived / 10
...
194 + 32 + 10 = 236 recorded mutants come back from the live gate (226 of them SCORED; the 10 no-coverage are excluded from the score). Every shape below exists because its ABSENCE lets a
