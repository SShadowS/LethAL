...
`tables.itest.ts`'s `EXPECTED` now asserts the live-measured **191 killed / 31 survived / 10
...
191 + 31 + 10 = 232 recorded mutants come back from the live gate (222 of them SCORED; the 10 no-coverage are excluded from the score). Every shape below exists because its ABSENCE lets a
