...
| `Data Blank Ops` (codeunit) | 79320 | `sandbox-data` | R159's assertion-screen arm: one `toggle-blank-string` site killed through `Library Assert`, so R121's screen SEPARATES it from that operator's two duplicate-key kills instead of flagging everything. |
| `Data Shift Ops` (codeunit) | 79325 | `sandbox-data` | R159's assertion-screen arm for `shift-integer`: a TWIN PAIR that controls FOUR operators at once. Its shape yields an `empty-block`, a `negate-conditional`, a `shift-integer` and a `return-value` mutant per half, all eight killed for the same reason, so the verdicts are constant and the only variable is which side of R121's screen each kill lands on. |
| `Hang Logic.NextRow` / `.WalkOneRow` | 79400 | `sandbox-hang` | R164's arm: BC's `Rec.Next()` CONTRACT with no table behind it (1 while rows remain, 0 once exhausted, 0 forever after), so the canonical `until Rec.Next() = 0` hang is deterministic. Its negated mutant was MEASURED `timeout-killed` before `loop-truncate`'s cession landed, which is the only time this repository has executed one. |
...
`tables.itest.ts`'s `EXPECTED` now asserts the live-measured **201 killed / 34 survived / 11
...
then took it to 84 deployed mutants — 93 raw specs, 9 dropped by dedup; R78 then to 87 deployed / 96 raw; R70 then to 90 deployed / 99 raw; R73 then to 103 deployed / 114 raw; R71 then to 106 deployed / 117 raw; R68 then to 106 deployed / 118 raw (a Tier-2 claim gained, its Tier-1 twin deduped away); R82 then to 136 deployed / 148 raw — `codeunit 79311 "Data Swap Ops"`, six arms for `lethal.swap-call-arguments`, and note that NONE of its 30 specs is deduped away: four sites carry a swap AND a `void-method-call` mutant, because dedup keys on replacement TEXT and a swap's is never a deletion's empty string; R72 then to 141 deployed / 154 raw — `Data Commit Ops.CommitThenRunValueForm`, whose six raw specs lose one Tier-1 `void-method-call` to the Tier-2 `remove-commit` at its `Commit()`; R136 then to 192 deployed / 207 raw — three operator changes (`swap-modify-flag` extended to `Insert`/`Delete`, plus the new `swap-find-direction` and `validate-to-assign`) across eleven fixture arms in two new tables, with all 51 per-mutant verdicts pre-committed and matching, and two of the 15 dedup drops newly explained by the same reversed-`SetRange`-displacement pattern the find arms share; R134 then to 224 deployed / 243 raw, `codeunit 79317 "Data Filter Ops"`, seven arms for the new `lethal.flip-filter-literal`, whose 36 raw specs lose FOUR to dedup and none of them the operator's own: at each of its six spans the Tier-2 splice and the Tier-1 `void-method-call` coexist, because a splice's replacement text is never a deletion's empty string, and the four drops are the ordinary `remove-setrange` displacements at arms C, D, E and H's scoping `SetRange`; R141 then to 228 deployed / 248 raw, one further arm in the same codeunit (`CountTaggedInBand`), whose `SetFilter` literal carries an inner quote and therefore hits the operator's CHARACTER refusal rather than arm H's ladder exhaustion: five raw specs, one `remove-setrange` displacement, four deployed mutants all killed, and `flip-filter-literal` still emitting exactly SIX mutants; R132 then to 232 deployed / 252 raw, `codeunit 79318 "Data Assert Ops"`, a TWIN PAIR of identically-shaped procedures whose covering tests differ only in how they raise (one through Microsoft's `Library Assert`, one through bare `Error(...)`), which is what moves this gate's `assertionScreen.discrimination` from `vacuous` to `partial` -- the branch of R121's screen no live gate had ever exercised. The test app gained a `Library Assert` dependency for it; the `vacuous` case moved to `itest:bcdev`, which now pins it; R161 then to 236 deployed / 256 raw, adding NO fixture code at all -- six operators' guard widened from `isStatementPosition` to `isStatementSlot`, so the un-braced body of a branch is claimed, and four `if <bad condition> then Error(...)` guard clauses already in this fixture became mutable for the first time. All four are `void-method-call`, none displaces anything, and the one at `Data Main`'s `Category` `OnValidate` shares its statement with the existing `remove-calcfields` mutant and becomes its SIBLING in one dispatch chain** (see §"Extension objects"
below). Both numbers are reproducible offline, no server needed: `generateMutationSet` returns 252
(which `tables.itest.ts` asserts before it deploys anything), `dedupeSpecs` drops 20, and
201 + 34 + 11 = 246 recorded mutants come back from the live gate (235 of them SCORED; the 11 no-coverage are excluded from the score). Every shape below exists because its ABSENCE lets a
...
three objects that close that, and the frozen figures moved 64/9/2 → 69/9/6 because of them (then → 69/9/9 with R78's TestPage-only pair, then → 71/9/10 with R70's cross-kind name collision).
...
| `page "Data Main List"` (`DataMainList.Page.al`) | Deliberately code-free — it is only a host, and contributes zero mutation sites. Since R144 it also carries one DECLARATIVE site: `Enabled = Rec."Modify Count" > 0`, a page property whose value is a boolean expression. `lethal.conditional-boundary` claims it, `isMutableSite` drops it, and it reaches the report as `declarativeSites` rather than as a mutant — so the mutant totals are unchanged and the gate can assert a real 1 instead of a 0 that would read the same on a build where the count never arrived. | 0 mutants, 1 declarative site refused |
...
halves are **R69**, now CLOSED as measured-unprofitable at 2.30% of a real app's mutants (see `docs/measurements`). The file-count sizing below is superseded and kept only because the number circulated: 9 of Document Output's 104 test files declare
a TestPage.
