...
- **al-runner**: killed **3** / survived **12** / no-coverage **4**, baseline green, per-mutant baseline matches `packages/runner/itest/al-runner.baseline.json`, and since R220 (2026-09-09) the table equals bcdev's PER MUTANT. The 3 kills are the same three bcdev kills (`IsOverBudget`, killed by `OverBudgetDetected`, a bare `Error(...)` test). The 4 no-coverage mutants are `SandboxPricing`'s: until R220 wired al-runner's own `--coverage`, this backend ran them without coverage and reported them survived (3 / 16 / 0). All four legs (one-shot, rerun, `--server`, resource selector) must match per mutant; the platform-app pin is asserted on the two one-shot legs only, since `--server` never receives it (R235). Version under the freeze: 3 / 12 / 4 was measured on v2.11.0 (R220, 2026-09-09); on 2026-09-26 the same build failed only the pin check (R235); after R235's fix, the same day on v2.11.0: PASS, 19 verdicts identical on all four legs, pin engaged on both one-shot legs. The v1-era `asserterror` defect (R7) is fixed upstream: the canary reports `defect-not-reproduced` each session, and this fixture never exercised it either way.
