...
- **al-runner**: killed **3** / survived **13** / no-coverage **0** (18.8%), baseline green, per-mutant baseline matches `packages/runner/itest/al-runner.baseline.json` — **measured against al-runner v2.0.1.0** (2026-08-07). These are the same figures v1.0.31 produced, which is the point: R93 pre-committed that prediction before the port was written, and the adapter moved no verdict.
