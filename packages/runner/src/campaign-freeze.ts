/**
 * Archive a stage's report and freeze its per-mutant verdicts to a committed file (Task 3, DO live
 ...
 * so a stage's evidence has to be copied out to a COMMITTED path and diffed against a COMMITTED
 * baseline before the next stage is allowed to start.
 *
 ...
 * archived `<stage>.report.json` and `<stage>.baseline.json` always describe the same per-mutant
 * verdicts; a mismatching report is archived under `<stage>.mismatch[-n].report.json` instead of
 ...
 * truncated report freeze itself as its own baseline on the very first stage, and every later stage
 ...
 * through `freezeStageTo` directly, bypassing this default entirely.
 */
...
/**
 * The records directory this campaign's zero-arg `freezeStage` defaults to, resolved against the
...
/**
 * Same contract as `freezeStage`, with the records directory injected rather than resolved from
 ...
 */
export async function freezeStageTo(
  reportPath: string,
  stage: string,
  ...
  // the copy first, run 2's report overwrote `<stage>.report.json` while `<stage>.baseline.json`
  ...
  // evidence. With the compare first, `<stage>.report.json` is only ever written after
  ...
  try {
    ...
    const dest = mismatchDestination(recordsDir, stage);
    await copyFile(reportPath, dest);
    const detail = err instanceof Error ? err.message : String(err);
    throw new Error(
      `${stage} freeze: the report does not match the committed baseline. ${stage}.report.json was NOT overwritten (it still holds the run the baseline was recorded from); the mismatching report is archived at ${dest}.\n${detail}`,
      { cause: err },
    );
  }
  await copyFile(reportPath, join(recordsDir, `${stage}.report.json`));
  console.log(`[freeze] ${stage}: ${report.mutants.length} mutants archived and frozen`);
}

/**
 * First free `<stage>.mismatch[-n].report.json`. A fixed name would let a second failing run
 * destroy the first one's evidence — the exact loss this module exists to prevent, one level down.
 */
function mismatchDestination(recordsDir: string, stage: string): string {
  for (let n = 1; n <= 100; n++) {
    const p = join(
      recordsDir,
      n === 1 ? `${stage}.mismatch.report.json` : `${stage}.mismatch-${n}.report.json`,
    );
    if (!existsSync(p)) return p;
  }
  throw new Error(
    `campaign-freeze: 100 mismatching ${stage} reports are already archived in ${recordsDir}. Refusing to overwrite evidence — clear them out deliberately.`,
  );
}
...
export async function freezeStage(
  reportPath: string,
  stage: string,
  expectedCount: number,
): Promise<void> {
  await freezeStageTo(reportPath, stage, expectedCount, defaultRecordsDir());
}
