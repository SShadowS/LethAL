#!/usr/bin/env bun
/**
 * R83 probe driver — same shape as `scripts/probe-r1-permissions.ts` (calls `LethALControl_RunMutant`
 ...
 * last had published.
 *
 *   bun scripts/r83-probe/drive.ts <config.json> <targetAppId> <codeunitId> <method> [...]
 */
...
if (!configPath || !targetAppId || !codeunitRaw || methods.length === 0) {
  throw new Error(
    "usage: bun scripts/r83-probe/drive.ts <config.json> <targetAppId> <codeunitId> <method> [...]",
  );
}
...
const acquire = await postJson("AcquireLease", {
  owner: "probe-r83",
  ttlSeconds: 300,
  clientNonce: `r83-${process.pid}-${performance.now()}`,
