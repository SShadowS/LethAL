#!/usr/bin/env bun
/**
 * R72 probe driver — same shape as `scripts/probe-r1-permissions.ts` (calls `LethALControl_RunMutant`
 ...
 * last had published.
 *
 *   bun scripts/r72-probe/drive.ts <config.json> <targetAppId> <codeunitId> <method> [...]
 */
...
if (!configPath || !targetAppId || !codeunitRaw || methods.length === 0) {
  throw new Error(
    "usage: bun scripts/r72-probe/drive.ts <config.json> <targetAppId> <codeunitId> <method> [...]",
  );
}
...
const acquire = await postJson("AcquireLease", {
  owner: "probe-r72",
  ttlSeconds: 300,
  clientNonce: `r72-${process.pid}-${performance.now()}`,
...
try {
  for (const method of methods) {
    ...
    const { status, text } = await post("RunMutant", {
      targetAppId,
      artifactId,
      attemptId: `r72-${opSeq}-${process.pid}`,
      mutantId: "",
