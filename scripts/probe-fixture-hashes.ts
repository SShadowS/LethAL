// scripts/probe-fixture-hashes.ts, the IDENTITY-HASH half of a grammar bump's proof.
//
// A bump can leave every SITE identical and still move every mutant's identity: 4.0.0 made
// `assignment_operator` a named node, `astSubtreeHash` serializes named children, and 32 fixture
// hashes moved with it, forcing all four gate baselines to be re-recorded. Site censuses cannot
// see that, so this emits the hash per surviving spec and a bump diffs the two listings.
//
// DERIVED FROM `census-fixture-mutants.ts` by adding `astHash=` to its output line, deliberately:
// it must replicate the real planning pipeline (targets -> generate -> validateSpec ->
// isMutableSite -> dedupeSpecs -> canCarryMutationSelectorVar) exactly, and a hand-written
// reimplementation got `validateSpec`'s signature wrong on the first attempt, which is the R215
// mistake of measuring a shortcut instead of the pipeline. Keep the two in step.
