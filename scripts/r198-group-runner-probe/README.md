...
| race | `drive-stop-race.ps1`, 30 rounds alternating group/loop (`stop-race.measured.txt` holds the 20 recorded ones; the first 10 were console-only, all 408) | **32 of 33 stops answered 408** naming the AL StopSession call; the **one 400** ("Cannot establish a connection to the SQL Server/Database") was E8 of the committed run, GROUP mode, about 10 s after a fresh publish. Filed as [[R202]]: today's transport quarantines on it. A dead session's progress row never advances, which is the termination evidence the R198 redraft uses instead of the stop's own answer. |
