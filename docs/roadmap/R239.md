...
literal-in-condition shapes across 725 `repeat` loops on both reference corpora. That count is
`repeat`-only; the `while` forms (`while Go or false`, `while not true`) were not part of it and
were only checked by grep over `fixtures/` and `examples/`, where none turned up. **The body-guard
shape was not counted**, so it has no site figure; the next corpus census should count both the
`while` forms and the body-guard shape. That is why this
...
Two related shapes were found in review, both zero sites. A comment inside the parentheses,
`while (/*c*/ false) do` or `until (/*c*/ true)`, still gets an equivalent mutant from
`loop-truncate` / `loop-skip`, because their parenthesis unwrap takes the first named child, which
is the comment; `flip-boolean-literal` refuses these shapes correctly. This predates GH-07. And on a
loop whose only exit is in the body (`while true do begin I += 1; if I > 3 then exit; end`),
`remove-assignment` of the counter update makes the loop never end, and no hang tag covers a
body-exit loop; this row's body-guard paragraph describes that gap only for
`flip-boolean-literal`. The general hazard, a counter-driven loop made non-terminating by
`remove-assignment` (also by `swap-additive` and by `shift-integer`), is already recorded in
[[R159]].
