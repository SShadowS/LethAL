...
**Revision 3 (2026-09-25),** after review round 2: decision 1 states link 3 as a named trust assumption (approved by the owner 2026-09-25) instead of a byte proof, and points per-mutant proof at GH-24; Task 8's fixture forwards `attach` and writes real `.app` bytes; `inLease` without a lease is refused; the red-baseline assertion slices the trace at the first mutant activation.
...
**What this plan checks, and what it trusts** (trust assumption approved by the owner 2026-09-25). This is NOT a proof that the installed bytes contain the named guards. It is two checked links, one named trust assumption, and one run-time observation. None of it is supplied by the caller:
...
6. **Part B order change (2026-09-26, from the C02-05 plan review, `H:/lethal-coord/reviews/C02-05-plan/review-r1.md` finding 2).** `runNamedMutants` calls `attach` TWICE: a PREFLIGHT `attach` before `inLease`, so a server reporting another target is refused before anything on it is modified, and the mandatory `attach` after `rebindBackend`, as decision 4 already has it. The order is: lease, preflight `attach`, `inLease`, rebind, `attach`, baseline, covering, release. Task 8's order test expects both `attach` entries; its "wrong installed artifact" test sees the refusal at the preflight, before `inLease` runs.
