...
5. **Decision 1's trust assumption waits on the owner** (coord question `q-20260925T195840-0432c6fb`). Part A does not depend on it and may start. Part B (C02-04b) must not start until that question is answered; if the owner requires byte-level proof, the orchestrator revises Decision 1 first.
