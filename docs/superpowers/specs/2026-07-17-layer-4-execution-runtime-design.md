...
- **`deploy()`:** `publisher.ts` shells `alc.exe` (path from `alcPath` config, default: newest `ms-dynamics-smb.al-*` under `~/.vscode/extensions`) with the target project's `.alpackages` as symbol cache, then `altool.exe publishapp` with schema sync `ForceSync` (re-batches replace guard sites; Add-only is insufficient). The instrumented app keeps the target app's id and version-bumps per batch (`1.0.<run>.<batch>`) so the test app's dependency stays valid; it replaces any dev-published version — fixture owns this risk, real projects get a docs warning. **Component order is load-bearing (corrected 2026-07-18 against a live server):** BC refuses to publish a version lower than the one already installed, so the original `1.0.<batch>.<run>` was non-monotonic across runs (run 5 batch 0 = `1.0.0.5` < run 4 batch 2 = `1.0.2.4`) and every batch below the previous run's highest index failed to deploy. `run` must come first. **The target app must also not be published Global/PerTenant:** `altool publishapp` posts to `/BC/dev/apps` (Development scope) and BC rejects it outright when a Global copy of the same app id exists. Compile stderr surfaced verbatim; compile or publish failure aborts the batch.
