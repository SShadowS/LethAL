...
- `LETHAL_ITEST_TABLES=1 bun run itest:tables` — the TABLE fixture (`fixtures/sandbox-data` + `-tests`), where Tier-2 operators and table-trigger mutation live. Frozen: killed **64** / survived **9** / no-coverage **2** over 75 deployed mutants (81 raw specs), plus `untargetedTriggerCount` **0**. Needs a gitignored `fixtures/sandbox-data/lethal.config.local.json` — **not** sandbox-app's; the two fixtures target different containers.
