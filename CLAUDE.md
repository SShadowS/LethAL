...
## Environment
Git bash on Windows; use bash syntax with Windows paths. **Never `2>nul`** (creates undeletable files) — use `2>/dev/null`. Interactive host commands (container publish/unpublish) are run by the user via `! <cmd>`.
