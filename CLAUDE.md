...
- `LETHAL_ITEST_TABLES=1 bun run itest:tables` — the TABLE fixture (`fixtures/sandbox-data` + `-tests`), where Tier-2 operators and table-trigger mutation live. Frozen: killed **69** / survived **9** / no-coverage **6** over 84 deployed mutants (93 raw specs), plus `untargetedTriggerCount` **0**. (R30 grew it from 64/9/2 over 75: the fixture gained a `tableextension` — 5 new mutants, all killed — and a `page`/`pageextension` pair, whose 4 mutants are `no-coverage` because a `TestPage` hangs the fenced session, R69.) Needs a gitignored `fixtures/sandbox-data/lethal.config.local.json` — **not** sandbox-app's; the two fixtures target different containers.
